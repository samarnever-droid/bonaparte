...
fn test_circle_shape_rasterization() {
    ...
    assert!(
        center_px[0] > 0.9,
        "Circle center (50, 50) should be red, got: {:?}",
        center_px
    );
    ...
    assert!(
        inside_px[0] > 0.9,
        "Point (50, 30) inside circle radius should be red, got: {:?}",
        inside_px
    );
    ...
    assert!(
        corner_px[0] < 0.1,
        "Bounding box corner (25, 25) must be outside circle (black), got: {:?}",
        corner_px
    );
    ...
    assert!(
        outer_px[0] < 0.1,
        "Outer point (10, 10) must be black, got: {:?}",
        outer_px
    );
}
...
fn test_rounded_rect_shape_rasterization() {
    ...
    assert!(
        corner_px[1] > 0.9,
        "Rect corner at (25, 25) must be green, got: {:?}",
        corner_px
    );
}
